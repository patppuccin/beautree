#!/usr/bin/env bash
# beautree Installation Convenience Script for Linux & macOS
# Usage: bash <(curl -fsSL https://raw.githubusercontent.com/patppuccin/beautree/main/scripts/install.sh)
...
install_beautree() {
    ...
    local archive_name="beautree-$os-$arch.tar.gz"
    local checksum_file="beautree_${clean_version}_checksums.txt"
    ...
    log_inf "Installing beautree $version for $os/$arch"
    ...
    log_inf "Installed beautree $version to $INSTALL_DIR/$BIN_NAME"

    if [[ ":$PATH:" != *":$INSTALL_DIR:"* ]]; then
        log_wrn "$INSTALL_DIR is not in PATH"
        log_wrn "Add the following to your shell profile:"
        log_wrn "  export PATH=\"\$HOME/.local/bin:\$PATH\""
    fi

    echo ""
    printf "\033[34mNext steps:\033[0m\n"
    echo "  1. Ensure $INSTALL_DIR is in your PATH"
    echo "  2. Run: beautree --help"
	echo "  3. Optionally alias to tree: alias tree=beautree"
}

echo ""
printf "\033[34mbeautree Linux/macOS Installer\033[0m\n"
